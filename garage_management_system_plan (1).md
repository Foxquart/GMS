...
- One admin/owner uses the system.
...
The owner should be able to create a job in seconds.
...
For a single-admin V1, keep authentication simple. A secure session/cookie-based implementation is sufficient.
...
- There is one admin user initially.
...
Even though V1 has exactly two locations, use a location table instead of hardcoding columns such as `shop_stock` and `warehouse_stock`.
...
1. One admin/owner is enough for V1.
...
1.client want to manage shop and house stock 